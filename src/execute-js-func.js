 ...
 * @private
 */
async function executeMainProcessMethod(includeModules, moduleName, methodChain, args) {
    ...
    try {
      if (receive.eval) {
        ...
        if (parts.length > 1 && parts[0] === requireElectronModule) {
          receive.result = await executeMainProcessMethod(options.mainProcessModules, parts[1], parts.splice(2), receive.args);
        } else {
